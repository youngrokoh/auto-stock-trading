...
독립 일회성 컨테이너는 접근 토큰을 다시 요청한다. 즉시 재실행에서 `/oauth2/tokenP` HTTP `403`이 발생하면 추가 호출을 반복하지 말고 1분 이상 기다린 뒤 한 번만 재실행한다. 2026-08-14 검증에서는 보호 구간 이후 재실행이 성공했다. 주기 수집 전에는 프로세스 간 토큰 저장소를 추가한다.
