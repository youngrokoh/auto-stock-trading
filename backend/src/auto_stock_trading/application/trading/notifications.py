...
from auto_stock_trading.application.trading.startup import reset_automation_on_start
...
class FillNotificationListener:
    ...
    async def reset_on_start(self, now: datetime) -> AutomationState:
        ...
        세션 내부 재연결은 프로세스 시작이 아니므로 리스너 기동에서만 호출한다. 규칙 자체는 API
        서버와 공유한다 — 리스너는 기본 구성에 없어서, 여기에만 두면 기본 구성이 재시작에서 상태를
        되돌리지 않는다(2026-08-24 실측).
        """
        return await reset_automation_on_start(self.orders, self.environment, now)
