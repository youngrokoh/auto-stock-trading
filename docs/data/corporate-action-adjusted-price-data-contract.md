...
- 배당·분배락일 확정 경계는 사용자가 2026-08-17 승인한 규칙 기반 도출을 사용한다. KRX T+2 결제 규칙에 따라 `배당락일 = 기준일 이전 마지막 거래일의 직전 거래일`로 도출하고, 검증된 `XKRX` 시장 달력만 사용하며 달력이 없는 기간은 확정하지 않고 `pending`을 유지한다. 확정은 원본 근거를 재사용하는 새 사실 버전(quality `verified`)으로 기록하고, KIS 수정주가 대조(검증 시나리오 9)로 사후 검증한다.
- 수정주가 데이터셋은 Alembic `20260817_0007`의 세 테이블에 발행 입력 유일키, `building`·`published` 부분 유일 인덱스, 계수·기간 제약으로 저장한다. 입력 해시는 `거래일:행ID:버전`(일봉)과 `사건일:action_key:버전`(기업행사)을 UTF-8 행으로 연결한 SHA-256이며 테스트 벡터로 고정했다. 계산기는 `Decimal` 정밀도 34로 계산하고 가격 8자리·계수 16자리 `ROUND_HALF_UP`으로 저장하며, 같은 입력 반복은 기존 `published`를 반환하고 입력 정정은 이전 발행본을 `superseded`로 표시한 채 새 데이터셋을 만든다.
- 같은 효력일의 주식 수 변화와 현금배당 조합은 공시의 주당 현금 기준을 확인할 수 없어 계약대로 생성을 실패시킨다. 미확정 일봉, 설명 없는 누락 일봉, 충돌·미지원·조건 누락 기업행사는 `failed` 데이터셋과 비밀정보 없는 오류 코드로 발행을 차단한다.
- 수정주가 읽기 API는 종목별 기업행사 현재·이력·`knowledge_cutoff_at` 시점 조회, 종목·방식별 최신 `published` 데이터셋과 파생 일봉·개별 계수, 데이터셋 ID 조회와 기업행사별 영향 데이터셋 목록을 제공한다. 응답은 `method`, `price_cutoff_date`, `knowledge_cutoff_at`, `algorithm_version`, 두 입력 해시, 데이터셋 ID와 일봉·기업행사별 출처를 포함하고, 파생 일봉마다 비수정 일봉 버전과 반영 기업행사 버전 계보를 노출하며 원본·수정 여부를 불리언 하나로 표현하지 않는다. 기간 필터는 `coalesce(ex_date, effective_date)` 기준이므로 락일 확정 전 버전은 기간 조회에 나타나지 않고 기간 없는 조회로 확인한다.
- 액면분할·감자 등 나머지 유형 수집과 출처 충돌 판정은 아직 구현 전이다.
...
4. 완료: 배당·분배락일 확정 경계와 수정계수 계산기·데이터셋·수정 일봉 저장소
5. 완료: point-in-time 조회와 읽기 API
