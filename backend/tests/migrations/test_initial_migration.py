from typing import TYPE_CHECKING

from alembic import command
from alembic.config import Config

if TYPE_CHECKING:
    import pytest


def test_initial_migration_renders_reproducible_postgres_sql(
    capsys: pytest.CaptureFixture[str],
) -> None:
    command.upgrade(Config("alembic.ini"), "head", sql=True)

    migration_sql = capsys.readouterr().out
    expected_snippets = (
        "CREATE SCHEMA IF NOT EXISTS reference",
        "CREATE SCHEMA IF NOT EXISTS market",
        "CREATE SCHEMA IF NOT EXISTS operations",
        "CREATE TABLE reference.instrument",
        "CREATE TABLE operations.raw_api_response",
        "CREATE TABLE market.quote",
        "CREATE TABLE market.market_bar",
        "CONSTRAINT uq_market_bar_identity UNIQUE",
        "CONSTRAINT uq_market_bar_version UNIQUE",
        "CREATE UNIQUE INDEX uq_market_bar_current",
        "CONSTRAINT ck_market_bar_finality",
        "CONSTRAINT ck_market_bar_confirmation",
        "CONSTRAINT ck_market_bar_validity",
        "CREATE TABLE reference.market_calendar",
        "CONSTRAINT ck_market_calendar_session_window",
        "CREATE UNIQUE INDEX uq_market_calendar_current",
        "CREATE TABLE operations.scheduled_job_run",
        "CONSTRAINT uq_scheduled_job_execution UNIQUE",
        "CONSTRAINT ck_scheduled_job_state",
        "CREATE TABLE reference.etf_index_classification",
        "CONSTRAINT uq_etf_index_classification_version UNIQUE",
        "CREATE UNIQUE INDEX uq_etf_index_classification_current",
        "CREATE TABLE market.corporate_action",
        "CONSTRAINT uq_corporate_action_version UNIQUE",
        "CONSTRAINT uq_corporate_action_source_event UNIQUE",
        "CREATE UNIQUE INDEX uq_corporate_action_current",
        "CONSTRAINT ck_corporate_action_type",
        "CONSTRAINT ck_corporate_action_lifecycle",
        "CONSTRAINT ck_corporate_action_quality",
        "CONSTRAINT ck_corporate_action_time_precision",
        "CONSTRAINT ck_corporate_action_share_multiplier",
        "CONSTRAINT ck_corporate_action_amounts",
        "CONSTRAINT ck_corporate_action_version",
        "CONSTRAINT ck_corporate_action_validity",
        "CREATE TABLE market.adjustment_dataset",
        "CONSTRAINT ck_adjustment_dataset_method",
        "CONSTRAINT ck_adjustment_dataset_status",
        "CREATE UNIQUE INDEX uq_adjustment_dataset_inputs",
        "CREATE TABLE market.adjustment_dataset_action",
        "CONSTRAINT ck_adjustment_dataset_action_factors",
        "CREATE TABLE market.adjusted_market_bar",
        "CONSTRAINT uq_adjusted_market_bar_date UNIQUE",
        "CONSTRAINT ck_adjusted_market_bar_factors",
        "CREATE TABLE market.minute_bar",
        "CONSTRAINT uq_minute_bar_version UNIQUE",
        "CREATE UNIQUE INDEX uq_minute_bar_current",
        "CONSTRAINT ck_minute_bar_prices",
        "CONSTRAINT ck_minute_bar_amounts",
        "CONSTRAINT ck_minute_bar_finality",
        "CONSTRAINT ck_minute_bar_confirmation",
        "CONSTRAINT ck_minute_bar_version",
        "CONSTRAINT ck_minute_bar_validity",
        "CREATE TABLE fundamental.financial_report",
        "CONSTRAINT uq_financial_report_version UNIQUE",
        "CONSTRAINT uq_financial_report_receipt UNIQUE",
        "CREATE UNIQUE INDEX uq_financial_report_current",
        "CONSTRAINT ck_financial_report_reprt_code",
        "CONSTRAINT ck_financial_report_fs_div",
        "CONSTRAINT ck_financial_report_year",
        "CONSTRAINT ck_financial_report_version",
        "CONSTRAINT ck_financial_report_validity",
        "CREATE TABLE fundamental.financial_statement_line",
        "CONSTRAINT uq_financial_statement_line_seq UNIQUE",
        "CONSTRAINT ck_financial_statement_line_seq",
        "CONSTRAINT ck_financial_statement_line_sj_div",
        "CREATE TABLE reference.listed_share_count",
        "CONSTRAINT uq_listed_share_count_version UNIQUE",
        "CREATE UNIQUE INDEX uq_listed_share_count_current",
        "CONSTRAINT ck_listed_share_count_positive",
        "CONSTRAINT ck_listed_share_count_version",
        "CONSTRAINT ck_listed_share_count_validity",
        "CREATE TABLE market.investor_flow",
        "CONSTRAINT uq_investor_flow_version UNIQUE",
        "CREATE UNIQUE INDEX uq_investor_flow_current",
        "CONSTRAINT ck_investor_flow_version",
        "CONSTRAINT ck_investor_flow_validity",
        "CREATE TABLE fundamental.disclosure",
        "CONSTRAINT uq_disclosure_receipt UNIQUE",
        "CONSTRAINT ck_disclosure_type",
        "CREATE TABLE strategy.backtest_run",
        "CONSTRAINT ck_backtest_run_range",
        "CONSTRAINT ck_backtest_run_initial_cash",
        "CONSTRAINT ck_backtest_run_status",
        "CONSTRAINT ck_backtest_run_failure_code",
        "CREATE TABLE strategy.backtest_trade",
        "CONSTRAINT uq_backtest_trade_sequence UNIQUE",
        "CONSTRAINT ck_backtest_trade_action",
        "CONSTRAINT ck_backtest_trade_execution",
        "CREATE TABLE strategy.backtest_equity",
        "CONSTRAINT uq_backtest_equity_date UNIQUE",
        "CREATE TABLE trading.account_snapshot",
        "CONSTRAINT ck_account_snapshot_amounts",
        "CREATE TABLE trading.account_position",
        "CONSTRAINT uq_account_position_snapshot UNIQUE",
        "CONSTRAINT ck_account_position_quantity",
        "CREATE TABLE trading.automation_state",
        "CONSTRAINT uq_automation_state_environment UNIQUE",
        "CONSTRAINT ck_automation_state_state",
        "CREATE TABLE trading.automation_event",
        "CONSTRAINT ck_automation_event_type",
        "CREATE TABLE trading.order_plan",
        "CONSTRAINT ck_order_plan_status",
        "CONSTRAINT ck_order_plan_block_code",
        "CREATE TABLE trading.order",
        "CONSTRAINT uq_order_client_order_id UNIQUE",
        "CONSTRAINT uq_order_plan_sequence UNIQUE",
        "CONSTRAINT ck_order_state",
        "CONSTRAINT ck_order_planned_fields",
        "CONSTRAINT ck_order_reject_code",
        "CREATE TABLE trading.order_event",
        "CONSTRAINT uq_order_event_sequence UNIQUE",
        "CREATE TABLE trading.risk_decision",
        "CONSTRAINT uq_risk_decision_rule UNIQUE",
        'ALTER TABLE trading."order" ADD COLUMN broker_org_no VARCHAR(8)',
        'ALTER TABLE trading."order" ADD COLUMN broker_order_time VARCHAR(6)',
        'ALTER TABLE trading."order" ADD COLUMN submitted_at TIMESTAMP WITH TIME ZONE',
        'ALTER TABLE trading."order" ADD COLUMN average_fill_price NUMERIC(24, 8)',
        "CREATE UNIQUE INDEX uq_order_broker_order_id",
        "ck_order_submitted_fields",
        "ck_order_fill_quantity",
        "reconcile_problem",
        "CREATE TABLE trading.fill_notification",
        "CONSTRAINT ck_fill_notification_kind",
        "CONSTRAINT ck_fill_notification_amounts",
        "CONSTRAINT ck_fill_notification_masked",
        "CREATE INDEX ix_fill_notification_broker_order",
        "CREATE TABLE trading.notification_session",
        "CONSTRAINT ck_notification_session_state",
        "CONSTRAINT ck_notification_session_end",
        "CREATE UNIQUE INDEX uq_notification_session_connected",
        "listener_state",
        "attestation",
        "ALTER TABLE trading.risk_decision ADD COLUMN attempt INTEGER",
        "CONSTRAINT ck_risk_decision_attempt",
        'ALTER TABLE trading."order" ADD COLUMN revision_count INTEGER',
        "CREATE TABLE reference.stock_profile",
        "CONSTRAINT uq_stock_profile_version UNIQUE",
        "CONSTRAINT ck_stock_profile_version",
        "CONSTRAINT ck_stock_profile_validity",
        "CREATE UNIQUE INDEX uq_stock_profile_current",
        "CREATE TABLE reference.dart_corp_code",
        "CONSTRAINT uq_dart_corp_code_version UNIQUE",
        "CREATE UNIQUE INDEX uq_dart_corp_code_current",
        "ALTER TABLE strategy.backtest_run ALTER COLUMN instrument_id DROP NOT NULL",
        "CREATE TABLE strategy.backtest_run_instrument",
        "CONSTRAINT uq_backtest_run_instrument UNIQUE",
        "CREATE INDEX ix_backtest_run_instrument_symbol",
        "ALTER TABLE strategy.backtest_trade ADD COLUMN symbol VARCHAR(9)",
        "ALTER TABLE strategy.backtest_run ADD COLUMN input_report_version_hash VARCHAR(64)",
        "CREATE SCHEMA IF NOT EXISTS ml",
        "CREATE TABLE ml.model",
        "CONSTRAINT ck_model_embargo_covers_horizon",
        "CONSTRAINT uq_model_name_version UNIQUE",
        "CREATE TABLE ml.model_evaluation",
        "CONSTRAINT uq_model_evaluation_metric UNIQUE",
        "CREATE TABLE ml.feature_importance",
        "CREATE TABLE trading.notification_outbox",
        "CONSTRAINT uq_notification_outbox_event UNIQUE",
        "CONSTRAINT ck_notification_outbox_state",
        "CONSTRAINT ck_notification_outbox_severity",
        "CREATE INDEX ix_notification_outbox_pending",
        "CREATE TABLE trading.notification_watermark",
        "CONSTRAINT uq_notification_watermark_environment UNIQUE",
        "CREATE TABLE strategy.live_signal",
        "CONSTRAINT uq_live_signal_basis UNIQUE",
        "CREATE TABLE strategy.live_signal_target",
        "CONSTRAINT uq_live_signal_target_symbol UNIQUE",
        "'schedule_blocked'",
    )
    for snippet in expected_snippets:
        assert snippet in migration_sql
