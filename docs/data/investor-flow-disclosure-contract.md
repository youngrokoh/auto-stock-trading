...
- 상태: 구현 기준
- 작성일: 2026-08-17
- 개정 승인: 사용자가 2026-08-23에 유니버스 전 종목 수급 수집과 Taskiq 스케줄러 일일 예약(ADR-0006 패턴)을 승인
...
- 실측 한계 (원본 보정 금지, 그대로 기록):
  ...
  3. 최근 약 30거래일만 반환한다. 과거 백필은 불가능하며 수집 시점부터 축적한다.
     최초 수집 이전 이력의 공백은 영구적이다. **다만 30거래일 창은 여유이기도 하다** —
     며칠 놓쳐도 다음 실행이 그 구간을 메운다. 한 달 넘게 거르면 그때부터 영구 공백이다.
  4. **특정 종목·거래일의 투자자 필드가 전부 빈 문자열로 온다**(실측 2026-08-23, 한화 `000880`의
     2026-08-21). 0으로 채우면 "순매수가 0이었다"는 시장에 대한 주장이 되므로 그 거래일 행을
     만들지 않고, 건너뛴 거래일을 수집 결과에 노출한다. 한 행 때문에 종목 전체를 실패로 돌리면
     축적 설계에서 그 종목이 영구히 막힌다.

### 수집 범위와 반복 (2026-08-23)

- 대상은 [종목 유니버스 계약](stock-universe-data-contract.md)이 정의한 KOSPI 200 전 종목이다.
  종목당 요청 1회이며 개별 실패는 기록하고 스윕을 계속한다 — 멈추면 그날 나머지 종목의 이력이
  영구히 빈다. 실패 종목은 개수와 종목코드를 함께 보고한다(무엇을 다시 돌릴지 알 수 있어야 한다).
- 서울 기준 당일을 제외하므로 어느 거래일을 얻으려면 **그 다음 날** 돌려야 한다. 예약은 이른
  아침(07:10·08:10·09:10 KST)에 시도하고, 성공하면 실행 claim이 나머지 시도를 건너뛴다.
- **거래일 판정은 하지 않는다.** 휴장일 실행은 같은 30거래일을 다시 관측하는 멱등 동작이고, 그
  덕분에 놓친 날이 자동으로 복구된다. 달력을 전제로 두면 달력 미커버가 수급 축적을 막는다.
- 부분 실패는 예약 작업을 **실패로** 기록한다. 수집된 종목은 이미 저장돼 있고, 실패로 남겨야 같은
  날 다음 시도가 남은 종목을 채운다.
- 예약은 기본으로 꺼져 있고 `infra/compose.investor-flow-schedule.yaml` 오버라이드에서만 켠다.
  [ADR-0006](../decisions/0006-market-calendar-scheduling.md) 규칙을 그대로 지킨다: 스케줄러는
  자격증명을 받지 않고, 중복 방지는 프로세스 수가 아니라 PostgreSQL 실행 claim이 하며, worker는
  **모의 자격증명만** 받는다. 이 작업은 읽기 전용 조회이며 주문 경로와 무관하다.
...
0. 유니버스 스윕: 한 종목 실패가 스윕을 멈추지 않고 실패 종목코드가 보고된다. 투자자 필드가 빈
   거래일은 행을 만들지 않고 건너뛴 날짜로 노출된다.
0. 예약: 기본값은 꺼짐이다. 켠 상태에서 같은 서울 날짜의 두 번째 시도는 실행 claim으로 건너뛰고,
   부분 실패는 실패로 기록돼 같은 날 다음 시도가 남는다.
...
- 수급: `KisInvestorFlowAdapter`가 서울 기준 당일 행을 제외하고 정규화하며,
  ...
  백만원).
- 유니버스 수급 실수집(2026-08-23): `--collect-universe-investor-flows`로 200종목을 수집했다.
  첫 실행은 199 성공·1 실패(`000880`, 빈 필드 파싱 오류)였고 빈 행 건너뛰기를 구현한 뒤 재실행에서
  200종목 전부 성공했다. 적재 결과는 현재 버전 6,004행·200종목·2026-07-03~08-21(종목당 30거래일,
  기존 수집분이 있는 삼성전자만 34일)이다.
- 예약 실환경 검증(2026-08-24): 오버라이드로 worker·스케줄러를 띄우고 자격증명 분리를 확인했다 —
  worker에는 `kis_paper_app_key`·`kis_paper_app_secret` secret이 마운트되고 스케줄러에는 secret이
  하나도 없다(`AUTO_STOCK_KIS_ENVIRONMENT` 미설정 시 기본값이 `paper`이므로 모의 호스트를 쓴다).
  컨테이너 안에서 예약 경로를 실행해 `succeeded`를 받았고 실행 claim
  `universe-investor-flows:2026-08-24`가 3분 34초로 기록됐다. 주말 뒤 실행이라 최신 거래일은
  08-21에서 늘지 않았다(정상: 서울 기준 당일 제외 + 08-22·23 휴장).
- 중복 방지 실측(2026-08-24): 09:10 KST 크론이 `00:10:00.003 UTC`에 정확히 발화했고(컨테이너가
  UTC여도 `cron_offset: Asia/Seoul`이 올바르게 해석된다) **claim 행이 1개로 유지됐다.** 수동으로
