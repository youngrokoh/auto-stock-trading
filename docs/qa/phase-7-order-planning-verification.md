...
**비상정지의 이벤트 기록이 리스너와 경합한다.** `transition_order`가 주문 행을 잠그지 않고
`max(sequence)+1`을 계산하기 때문에, 리스너가 취소 확인 통보를 반영하는 것과 비상정지가
동시에 같은 시퀀스를 잡아 유일 제약을 위반한다. 이번 실행에서 실제로 발생했다(취소 자체는
증권사에 전달됐고 주문 2건은 `canceled`가 됐지만, 비상정지 명령은 예외로 끝났다).

권장 수정은 이벤트를 붙이는 경로가 주문 행을 `SELECT ... FOR UPDATE`로 잠그는 것이다. 비상
정지는 사람이 마지막으로 쓰는 통제 수단이므로 부분 실패를 남기면 안 된다.
