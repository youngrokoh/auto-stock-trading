...
Frontend: React 19 + TanStack Query + Tailwind 4, no router yet — `App.tsx` switches on `window.location.pathname` (`/` overview, `/market`, `/showcase`). The approved design lives in `src/styles.css` (semantic tokens, light default + `prefers-color-scheme: dark` per spec §8) and `src/components/` (app-shell, coordinate-cell, status-badge, safety-banner, chart-panes). Market-data responses are parsed with Zod `strictObject` in `src/lib/market-data.ts` (decimals arrive as strings, including pydantic's `"0E-8"` scientific form); indicators (SMA/RSI/MACD/Bollinger) are computed client-side in `src/lib/indicators.ts` from confirmed bars only. Dev-only diagnostics load dynamically in `devtools.ts` behind `import.meta.env.DEV`.
