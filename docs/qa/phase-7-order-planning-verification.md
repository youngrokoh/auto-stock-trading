...
## 게이트 미조정 판정 결함 수정 (2026-08-25)

정책은 "내부 주문·체결·잔고와 증권사 상태의 **미조정 건이 0건**"이라고 쓴다(전환 게이트 §3). 발생
이력이 아니라 지금 미조정인 건수다. 구현은 `reconcile_problem` 이벤트를 **해소 여부 없이 전부** 세고
있었으므로 정책을 잘못 읽었다 — **정책 변경이 아니라 결함 수정이며 새 ADR이 필요하지 않다.**

증상: 한 번이라도 문제가 생기면 이 조건은 다시 충족될 수 없다. 정책이 요구하지 않은 영구 차단이다.

수정: 문제가 참조한 주문이 종결 상태(`filled`·`rejected`·`canceled`·`expired`)면 그 발산은 닫혔으므로
세지 않는다. 확인할 주문이 없으면 **해소로 보지 않는다**(fail-closed).

실데이터 효과: **27건 → 9건.** 18건은 참조 주문이 이미 종결돼 있었다.

| 사유 | 참조 주문 | 건수 | 판정 |
|---|---|---|---|
| `NOTIFICATION_GAP` | filled·canceled | 15 | 해소 |
| `UNKNOWN_BROKER_ORDER` | filled·canceled | 3 | 해소 |
| `UNKNOWN_BROKER_ORDER` | 기록 없음 | 5 | 미해소 |
| `TERMINAL_STATE_CHANGED` | 기록 없음 | 3 | 미해소 |
| `NOTIFICATION_GAP` | 기록 없음 | 1 | 미해소 |

`PostgresGateReader`에는 통합 테스트가 없었다 — 결함이 그 조회에 있었으므로
`tests/trading/test_gate_reader_integration.py`를 새로 만들어 5건으로 고정했다(종결이면 0, 만료도 같게,
열린 주문이면 1, 참조 주문이 없으면 1, 잔재는 별도 집계).

### 남은 9건 — 사람 확인 경로로 정리 (2026-08-25, ADR-0018)

9건 모두 `broker_order_id`·`client_order_id` 어느 쪽으로도 우리 기록에 없는 증권사 주문번호다(검증 중
사람이 수동으로 낸 주문으로 보인다). **이것은 미집계 오류가 아니다** — 증권사 계좌에 우리가 모르는
주문이 있다는 것은 실제로 미조정 상태이고, 사람이 "시스템 밖에서 낸 주문"이라고 확인해 주기 전까지
참이다. 파생으로 풀 방법이 없다.

따라서 남은 것은 "숫자를 고치는 일"이 아니라 **사람이 문제 기록을 해소로 표시하는 경로**였다.
[ADR-0018](../decisions/0018-human-resolved-reconciliation-records.md)로 승인받아 구현했다.
...
그 뒤 주문번호 8개(문제 9건)를 정리했다. `0000025643`은 문제 2건이 하나의 해소로 덮였다 — 같은 번호의
관측 여러 건은 하나의 발산이므로 설명도 하나다.
...
`SEVERE_INCIDENTS`는 여전히 막는다 — 장애는 일어난 일이고 그 조건은 최근 20거래일 창이라 시간이 지나면
자연히 회복된다.
...
## 세 결정이 맞물리는 것을 실데이터로 확인 (2026-08-26)

하루 안에서 ADR-0017·0018·0019가 사람 개입 없이 이어지는 것을 관측했다. 이것이 각 결정을 따로 검증한
것보다 중요하다 — 결정들 사이의 틈이 실제로 메워지는지는 함께 돌려봐야 알 수 있다.

| 시각 | 사건 | 근거 결정 |
|---|---|---|
| 09:12:39 | 리스너 없이 비상정지 → `CONFIRMATION_UNOBSERVABLE` 2건, 요청은 나감 | ADR-0019 |
| 09:12~ | 게이트 미조정 0 → 2, 차단 조건 9 → 10 | ADR-0018 |
| 15:43:51 | `--sync` → `reconciled=matched expired=2` | ADR-0017 |
| 15:43~ | 미조정 2 → **0**, 차단 조건 10 → **9** | ADR-0018 |

```
0000006251  expired  SESSION_ENDED  당일 체결 합계 수량 0 금액 0
0000006274  expired  SESSION_ENDED  당일 체결 합계 수량 0 금액 0
미종결 주문: 0
```

**미조정이 사람 개입 없이 0으로 돌아갔다.** 아침의 문제 2건은 참조 주문이 종결되자 파생으로 해소됐다 —
`--resolve`를 쓰지 않았다. 그 사이 게이트는 실제 발산을 정직하게 차단하고 있었다. 예측을 미리 적어두고
대조했으며 네 값 모두 일치했다(`matched`·`expired=2`·미조정 0·차단 9).

여기서 확인된 성질 하나: **사람 확인 경로는 파생으로 풀리지 않는 것에만 쓰인다.** 오늘처럼 주문이
종결되는 정상 흐름에서는 사람이 아무것도 하지 않아도 되고, 8월 25일의 9건처럼 우리 기록에 없는
주문번호일 때만 사람이 필요하다. ADR-0018의 대상 제한이 의도대로 좁게 작동한다.

## 무인 제출 첫 실행 (2026-08-27 09:35 KST)

사용자가 아침 절차를 잊어 09:33에야 켰고, 그 덕에 **09:05 슬롯이 아무 기록도 남기지 않은 것**이
드러나 아래 §큐 결함을 찾았다. 큐를 고친 뒤 09:35 슬롯이 처음으로 끝까지 돌았다.

한 계획(09:35:03)에서 6건이 나왔다.

| 종목 | 구분 | 수량 | 결과 |
|---|---|---|---|
| 069500 KODEX 200 | 매수 ×3 | 4+4+1 = 9주 | 전량 체결 |
| 005930 삼성전자 | 매도 ×2 | 1+1주 | 전량 체결 |
| 133690 TIGER 미국나스닥100 | 매수 | — | **거절 `RISK_UNCLASSIFIED_EXPOSURE`** |

매수 3분할은 종목 한도 10%를 주문당 5%로 나눈 결과다. 거절은 한도 1,007,220원에 예상 1,168,305원으로,
**ETF에 업종 사실이 없어 미분류 한도가 걸린 것**이다 — 문서에 적힌 정책 그대로이며 위험 엔진이 사람
없이 제 역할을 했다.

체결 확정까지 15초 안에 끝났고 전 과정이 알림으로 나갔다(계획·거절·제출·체결).

### 오래 남아 있던 관측 하나가 해소됐다

```
0000010961  submitted → partially_filled → filled   09:35:17 → 09:35:18
```

**부분체결 전이**는 2026-08-19 실주문이 즉시 전량 체결되는 바람에 관측하지 못해 "다음 실주문에서
지정가를 벌려 확인한다"로 남아 있었다. 무인 실행에서 자연히 발생했다.

### 관찰 (원인 미상, 자기 복구함)

09:36:13에 리스너 세션이 끊기고 09:36:14에 다시 붙었다. 컨테이너는 재시작되지 않았고
(`RestartCount=0`) 로그에 `reason=STOPPED`가 남았다 — 리스너 루프가 스스로 끝나고 다시 시작한 것이다.
그 시점에 열린 주문이 없어 `NOTIFICATION_GAP`은 뜨지 않았고 체결 5건은 모두 수신됐다. **통보는
재생되지 않으므로 이 1초 공백이 열린 주문과 겹치면 확인이 유실된다.** 재발하면 원인을 파야 한다.

## 무인 운영 하루 전체 관측 (2026-08-28)

전날 고친 두 결함(큐 분리·재연결 백오프)이 하루 동안 어떻게 도는지 본 첫날이다. 결함 수정은 그 자리에서
확인했지만, **하루를 돌려봐야 알 수 있는 것**이 따로 있다.

```
15:57  --sync  →  reconciled=matched expired=0
계획 24건 · 차단 슬롯 0 · 미종결 0 · 알림 미발신 0 · 주문 1건(거절)
```

### 큐 분리

계획 24건. 직전 이틀은 6슬롯 중 4건, 그리고 09:05 슬롯 0건이었다. 슬롯이 다른 워커에 잡혀 사라지는
일이 없어졌다.

### 재연결 백오프

장중(09:00~15:30) 단절 8회가 **예외 없이 1초에 복구**됐다.

```
09:00:02→03  10:00:02→03  11:00:02→03  12:00:02→03
13:00:02→03  14:00:02→03  15:00:02→03  10:48:15→16
```

매시 정각 7회에 비정기 1회다. 고치기 전이라면 대부분 30초였고 그 안의 체결은 확인이 유실됐다.

### 장외 시간의 흔들림 (관찰, 장중 영향 없음)

같은 날 새벽에는 길게 끊겼다 — 04:10~04:21(11분), 06:54~07:23(29분), 07:24~07:55(31분). 06:54에는
20초 동안 8회 단절·재접속이 반복됐는데 대기가 1→2→4초로 올라간 것이므로 **백오프가 의도대로 동작한
것**이다. 30분대 공백은 대기가 아니라 재접속 시도가 계속 실패한 구간이다(연결이 서지 않으면 `ATTACHED`
이벤트가 남지 않는다).

전부 장 시작 전이고 **장중에는 한 번도 없었다.** KIS 웹소켓이 장외에 열려 있지 않은 것으로 보이며,
그 사이 미종결 주문이 0이라 `NOTIFICATION_GAP`도 발생하지 않았다. 판단이 필요한 것은 **장외에 리스너를
붙여둘 이유가 있는지**다 — 지금은 감사 테이블에 이벤트만 쌓인다(`listener_state`는 알림 대상이 아니라
푸시 도배는 없다).

### 주문이 거의 없던 이유

24슬롯에 주문 1건(거절)뿐이다. 전날 KODEX 200을 담고 삼성전자를 정리해 **목표 포지션에 이미 도달**했기
때문이다. 신호가 새 후보를 내지 않으면 `no_plan`이며 이것은 차단이 아니라 할 일이 없는 것이다 — 그래서
차단 이벤트를 만들지 않는다(인계 문서 131번). 거절 1건은 ETF 미분류 한도이며 **슬롯마다 반복되지
않았다**(24슬롯 중 1건). 반복 거절로 알림이 도배될 수 있다는 우려는 이 표본에서는 나타나지 않았다.
