...
지정가만 바꾸는 좁은 경로다([ADR-0011](../decisions/0011-paper-order-revision.md)). 수량은 그대로
두므로 목표 포지션이 바뀌지 않는다.
...
| 조건 | 위반 시 사유 코드 |
|---|---|
| 체결통보 리스너가 붙어 있다 | `LISTENER_NOT_ATTACHED` |
| 계좌 항등식이 성립한다(미체결 주문 존재는 조건이 아니다) | `ACCOUNT_NOT_RECONCILED` |
| 대상 주문이 `SUBMITTED`·`PARTIALLY_FILLED`이고 지점번호가 있다 | `NOT_OPEN` |
| 잔여 수량이 1주 이상이다 | `NOTHING_LEFT` |
| 주문 허용시간(09:05~15:15 KST) 안이다 | `MARKET_CLOSED` |
| 자동매매가 `RUNNING`이다 | `AUTOMATION_NOT_RUNNING` |
| 검증된 달력의 거래일이다 | `MARKET_CLOSED` |
| 기준가를 새로 조회했고 10초 안이다 | `DATA_STALE` |

### 판정

새 지정가는 기준가 대비 상대 버전트로만 계산한다(절대가 지정 경로 없음). 판정은 **새 주문과 같은
엔진·같은 규칙 코드**를 쓰며, 대상 주문 자신의 예상 노출과 미체결 카운트를 제외한다. 정정은 주문을
하나 더 만드는 것이 아니므로 자신을 두 번 세면 없는 위반을 만든다.

엔진이 같은 종목에 같은 수량 이상을 같은 가격으로 낼 수 있다고 판정해야 정정한다. 그렇지 않으면
해당 규칙 코드(예: `RISK_ORDER_AMOUNT`)로 거절한다.

**조정 완료 조건은 계획과 다르다.** 계획은 "계좌 항등식 성립 + 미체결 주문 없음"을 요구하지만,
정정은 미체결 주문에만 하는 동작이므로 미체결 존재를 이유로 막으면 경로가 성립하지 않는다
(2026-08-21 실측: 첫 정정 시도가 `ACCOUNT_NOT_RECONCILED`로 거절됐다). 정정은 **계좌
항등식만** 요구한다. 정책 §4의 요구를 완화한 것이 아니라 새 노출을 만드는 경로와 기존 주문을
바꾸는 경로를 구분한 것이다.

리스너 부착 조건은 제출과 같은 이유로 정정에도 적용된다([ADR-0009](../decisions/0009-realtime-fill-notification.md)
결정 3). 정정은 새 주문번호를 받고 체결되지 않던 가격을 체결 가능한 값으로 바꾸므로, 통보 없이
허용하면 장중에 확인할 수 없는 체결을 만든다.

### 요청과 응답

취소와 같은 엔드포인트를 쓰고 `RVSE_CNCL_DVSN_CD`만 `01`(정정)이다. 본문은 `KRX_FWDG_ORD_ORGNO`,
`ORGN_ODNO`, `ORD_DVSN`, `ORD_QTY`(잔여 수량), `ORD_UNPR`(새 지정가), `QTY_ALL_ORD_YN`(`Y`)이다.

| 응답 | 처리 |
|---|---|
| `rt_cd = 0` | `broker_order_id`를 **새 주문번호로 갱신**하고 지정가·정정 횟수를 갱신한다. 상태와 수량은 바꾸지 않는다. 원주문번호는 `order_revised` 이벤트에 남는다 |
| `rt_cd ≠ 0` | 주문을 바꾸지 않고 `order_revise_failed` 이벤트에 증권사 `msg_cd`를 남긴다 |
| 전송 실패 | `api_failure` 이벤트. 정책 §3의 연속 실패 규칙을 따른다 |

**주문번호 갱신이 필수다.** 정정 이후의 체결 통보는 새 주문번호로 오므로 갱신하지 않으면 그 체결이
대조되지 않는다(취소 통보 실측에서 확인한 구조).

### 감사

정책 §7이 모든 주문 시도의 규칙 판정을 요구하므로 정정 판정도 규칙별로 저장한다.
`trading.risk_decision`의 유일 제약에 회차(`attempt`)를 넣어 계획 시점(1회차)과 정정 시점(2회차부터)을
함께 보존한다(리비전 `20260820_0020`). `order.revision_count`는 정정 횟수다.
...
`RVSE_CNCL_DVSN_CD`(`02` 취소), `ORD_QTY`, `ORD_UNPR`(`0`), `QTY_ALL_ORD_YN`이다.
...
`QTY_ALL_ORD_YN`이 두 동작을 가른다. 전량 취소는 `Y`이고 이때 증권사는 `ORD_QTY`를 보지 않는다.
부분 취소는 `N` + 명시 `ORD_QTY`(취소할 수량)이며 나머지는 열린 채 남는다
([ADR-0013](../decisions/0013-paper-partial-cancel.md), 2026-08-24 실측).

- 부분 취소는 **사람이 CLI로** 대상 주문번호와 수량을 지정할 때만 일어난다
  (`--reduce --broker-order-id --quantity`). 재계획이 자동으로 부분 취소하는 경로는 없다.
- **위험검사·리스너 부착·자동매매 상태를 요구하지 않는다.** 노출을 줄이는 동작은 정책 §3·§4의 어떤
  한도도 새로 위반할 수 없고, 통보가 끊긴 상황에서 위험을 줄일 수단을 막으면 안 된다. 비상정지가
  리스너 조건 없이 미체결 전량 취소를 수행하는 것과 같은 논리다.
- 잔여 수량을 넘는 요청은 전송 전에 `QUANTITY_EXCEEDS_OUTSTANDING`으로 거부한다. 증권사도
  `40430000`("취소수량이 취소가능수량을 초과합니다")으로 막으므로 이중 방어다.
- **요청만으로 내부 수량을 줄이지 않는다.** 취소 요청과 체결이 경합할 수 있으므로 수량은 증권사
  체결통보로 확인된 값만 반영한다. 요청 경로는 `partial_cancel_requested` 이벤트(취소 주문번호를
  detail에 포함)만 남기고, 거절은 `partial_cancel_failed`로 남긴다.
- 부분 취소는 자체 주문번호를 새로 받지만 **원주문번호가 살아 있다.** 정정과 달리 내부
  `broker_order_id`를 갱신하지 않는다 — 이후 체결·취소가 계속 원주문번호로 온다.
- 취소 확인 통보의 수량이 잔여를 덮으면 `CANCELED`, 그보다 적으면 상태를 유지하고 수량만 줄인다.
  잔여를 넘는 취소 통보는 `ORDER_QUANTITY_MISMATCH` 발산으로 자동매매를 정지한다.
...
- 부분 취소는 통보된 수량만 반영한다: 잔여보다 적으면 상태 유지 + 수량 축소, 잔여를 덮으면
  `CANCELED`, 잔여를 넘으면 `ORDER_QUANTITY_MISMATCH` 발산이다.
- 내부에 없는 주문번호가 조회되면 자동매매가 `PAUSED`로 전이하고 사유가 `ACCOUNT_NOT_RECONCILED`다.
- 실제 모의계좌에서 주문 제출 → 체결 동기화 → 취소가 한 거래일 안에 관측된다.

## 이 단계의 한계 (기록)

- 지정가 정정은 2026-08-20에 [ADR-0011](../decisions/0011-paper-order-revision.md)로 도입했다(위 정정
  절). 미체결 수량 축소는 2026-08-24에 [ADR-0013](../decisions/0013-paper-partial-cancel.md)으로
  도입했다(위 전량/부분 취소 절) — **수량을 바꾸는 정정은 증권사 API에 없으므로** 그 동작은 정정이
  아니라 부분 취소다. **부분 정정(일부만 가격 변경)과 자동 정정은 여전히 후속이다.** 전자는 주문
  하나가 두 가격으로 쪼개져 내부 모델과 어긋나고, 후자는 목표 포지션 재계산 규칙이 필요하다.
...
- 자동 스케줄 제출이 없으므로 신호 발생과 제출 사이 지연은 사람의 실행 시점에 의존한다. 경계는
  2026-08-24에 [ADR-0015](../decisions/0015-scheduled-order-submission.md)로 승인됐다(계획-제출 체인,
  15분 주기, 하루 한 번 사람이 켜기, 차단은 알림으로, 모의 전용). **구현은 보류다** — 계획 경로에
  신호 원천이 없어서다: `SignalCandidate`는 사람이 CLI로 지정하거나 기존 주문에서 가져오는 두 경로만
  있고, `OrderPlanner`는 후보를 입력으로 받는다. 신호 원천은 별도 ADR·승인 사항이다.
