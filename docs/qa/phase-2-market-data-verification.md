...
| 실제 KIS 모의환경 | 로컬 모의 키로 두 번 수집하고 DB·API 확인 | 통과 |
...
독립 컨테이너를 즉시 재실행했을 때 새 프로세스의 토큰 발급이 HTTP `403`으로 한 번 거절됐다. 추가 호출을 반복하지 않고 보호 구간 이후 한 번 재실행해 성공했다. 현재 토큰 캐시는 프로세스 범위이므로 주기 수집 전 Valkey 기반 프로세스 간 재사용을 추가한다.
