...
따라서 남은 것은 "숫자를 고치는 일"이 아니라 **사람이 문제 기록을 해소로 표시하는 경로**이며, 게이트를
막는 조건을 사람이 해제하는 표면이므로 ADR과 승인이 필요하다.
...
- 우리 기록에 없는 증권사 주문 9건의 해소 경로(아래 절). 사람 확인이 필요한 새 경로이므로 결정 사항
