...
학습 진입점은 `worker/ml.py`이며 예약하지 않는다. `application/ml/training.py`가 로드·구간 분할·구간별 학습·평가·저장을 잇고, `adapters/database/ml_dataset_reader.py`가 확정 일봉만 읽어 특징과 목표를 만든다. 모델·구간 지표·특징 중요도는 `ml` 스키마(리비전 `20260822_0025`)에 한 트랜잭션으로 저장한다.
