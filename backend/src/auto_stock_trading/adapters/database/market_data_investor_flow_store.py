...
_SOURCE: Final = "KIS"
_OPERATION: Final = "investor_flows"


@final
class PostgresInvestorFlowStore:
    def __init__(
        self,
    ...
    @classmethod
    def from_url(cls, database_url: str) -> PostgresInvestorFlowStore:
        engine = create_async_engine(database_url, pool_pre_ping=True)
        return cls(engine, async_sessionmaker(engine, expire_on_commit=False))

    @classmethod
    def from_connection(cls, connection: AsyncConnection) -> PostgresInvestorFlowStore:
    ...
    async def save_flow_bundle(self, bundle: InvestorFlowBundle) -> None:
        async with self._sessions.begin() as session:
            ...
            raw_id = self._add_raw_row(session, bundle.raw)
            for flow in bundle.flows:
                await save_investor_flow(session, InvestorFlowEvidence(flow, instrument_id, raw_id))
            _ = await session.execute(
                sync_succeeded(
