...
선택지(구현하지 않았다. 어느 것도 정책 완화가 아니지만 감사·권한에 영향이 있어 승인이 필요하다):

| 방안 | 내용 | 유의점 |
|---|---|---|
| ㉠ 집계 기반 교차 확인 | `output2`의 `tot_ccld_qty`·`tot_ccld_amt`로 그날 우리 주문 합계를 대조 | 오늘처럼 계좌에 외부(수동) 주문이 섞이면 합계가 우리 주문만을 뜻하지 않는다 |
| ㉡ 사람이 확인한 관리 전이 | 사람이 KIS 화면에서 확인한 뒤 CLI로 확정하고 근거·실행자를 이벤트에 남긴다 | 증권사 응답이 아닌 사람의 확인이 근거가 된다. 감사 기록 설계가 필요하다 |
| ㉢ 웹소켓 단독 | 재대조를 포기하고 통보만 근거로 쓴다 | 통보 유실을 발견할 독립 경로가 없어진다. 실전 전환 게이트에 위험으로 남는다 |

이 결정 전까지 이 두 주문은 그대로 두고 자동매매는 `disabled`로 유지한다.
...
- **선행 조건이 미충족이다.** 2026-08-19 마감 후 `--sync`는 미체결 2건을 확정하지 못했다(이 TR이
  모의환경에서 주문별 행을 주지 않는다). 계획은 `ACCOUNT_NOT_RECONCILED`로 계속 차단되므로, 위
  "사용자 결정 대기" 절의 재대조 수단을 먼저 정하고 그 두 건을 정리해야 2단계 이후를 진행할 수 있다.
- 리스너 부착·단절 복구·제출 게이트 확인(1·4단계)은 이 결정과 무관하게 먼저 수행할 수 있다.
