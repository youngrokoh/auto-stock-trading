...
- 시장 달력 저장소는 누락·미확인·충돌·오래된 확인을 fail-closed로 판정한다. 승인된 [ADR-0006](../decisions/0006-market-calendar-scheduling.md)에 따라 서울 기준 KRX 선행 수집, KIS 당일 보완 확인과 PostgreSQL 영속 실행 claim을 구현했다. Compose의 단일 scheduler는 명시적 프로필에서만 시작하고 KRX 예약만 켠다. 실전 `CTCA0903R` 읽기 전용 검증은 통과했지만 KIS 자동 확인은 별도 사용자 활성화 전까지 비활성이다.
