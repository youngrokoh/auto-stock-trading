...
- e2e는 두 모드로 검증한다: 기본(빈 DB 구조·빈 상태, CI와 동일)과 `E2E_EXPECT_DATA=1`(실수집 데이터 검증). 2026-08-18에 3단계 대시보드 커밋부터 CI Quality가 실데이터 전제 때문에 연속 실패하던 것을 이 분리와 통합 테스트의 종목 자체 생성으로 복구했다.
