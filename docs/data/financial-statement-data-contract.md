...
- 개정 승인: 사용자가 2026-08-22에 사업보고서 소급 범위 확장(FY2016 사업연도부터, 소급 구간은 연간만)을 승인
...
- **연간 소급 깊이는 파라미터다**(`--annual-years`, 기본 5). 시점 정합 학습·백테스트는 과거 시점의 최신 사업보고서를 요구하므로 5개년으로는 그 시점에 보고서가 없는 구간이 생긴다. 2026-08-22 승인으로 FY2016부터 수집한다.
- 소급 구간은 **연간만** 수집한다(`--annual-only`). 분·반기를 함께 소급하면 요청 수가 3배가 되는데, 지표 계약이 분·반기 연환산을 금지하므로 쓰이지 않는 기간이다.
...
- **표준 계정 ID의 접두는 사업연도에 따라 다르다.** FY2018 이하 보고서는 `ifrs_`, FY2019 이후는 `ifrs-full_`을 쓴다(2026-08-23 실측: FY2016~2018 라인 60,844건이 `ifrs_`, FY2019 이후 0건). IFRS 택소노미 개정으로 접두만 바뀌었고 계정의 의미는 같다. `dart_` 접두는 변하지 않았다. **저장은 원문 접두를 그대로 보존하며**, 두 접두를 같은 계정으로 인정하는 것은 파생 지표 쪽 규칙이다([재무 지표 정의 계약](financial-indicator-contract.md) §입력 계정).
...
## 연간 소급 확장 실수집 결과 (2026-08-23)

- `--universe-statements --only-missing --annual-years 10 --annual-only`로 FY2016~2025 사업보고서를 수집했다. 200종목 전부 성공했고(실패 0, 요청 한도 초과 없음) 원본 저장 2,150건으로 요청 수가 검산된다: 전체 조합 10개년 × 2연결구분 × 200종목 = 4,000 중 현재 버전이 이미 있는 1,850(FY2021~2025)을 건너뛰고 2,150을 요청했다.
- 새 보고서 버전 1,516건이 저장되고 634건은 미제출(`status=013`)이었다(FY2016~2020 484건 + FY2021~2025에서 이전 스윕에도 없던 150건). 사업연도별 현재 버전(연결/개별)은 FY2016 137/141, FY2017 145/150, FY2018 150/155, FY2019 155/158, FY2020 162/163이다. 미제출은 대부분 그 시점에 상장·설립 전이거나 연결·개별 한쪽만 존재하는 경우다.
- **이 스윕은 `--only-missing`이므로 FY2021~2025의 정정 공시를 반영하지 않았다**(§유니버스 4의 명시된 대가). 정정 반영이 필요하면 기본 모드로 전량 재조회해야 한다.
- 확장 후 저장 총계는 보고서 4,162건(전부 `version=1`), 라인 792,884건이다. 기존 2,646건은 같은 접수번호 재조회 없이 그대로 남았다.
- 이 확장이 ML 재무 특징의 학습 초반 구간 결측을 메웠다. 측정 결과는 [ML 신호 계약 §재무 특징 실측](ml-signal-contract.md)에 있다.
