...
- **위험 판정의 입력이 되는 실패는 호출한 자리에서 기록한다.** 정책 §3의 "외부 API 5분 내 3회 실패"는 `automation_event`의 `api_failure` 행을 세므로, 기록되지 않은 실패는 그 규칙에 존재하지 않는 것과 같다. 계획 경로가 여러 계층(플래너 안의 조회, 신호 후보를 만들 때의 보유 조회)에서 외부를 호출하므로 **한 계층만 기록하면 나머지는 조용히 빠진다**(2026-09-04 실측: 전송 실패 4건에 `api_failure` 0건). 기록한 뒤에는 삼키지 않고 그대로 던진다 — 삼키면 실패한 실행이 할 일 없던 실행처럼 보인다.
- **예약 창은 기계가 자는 시간을 견뎌야 한다.** 이 시스템은 상시 서버가 아니라 사용자의 기계에서 돌고, 호스트가 자면 도커 VM과 함께 모든 예약이 선다(2026-09-04 실측: 06:10 이전 어떤 스케줄러도 발송하지 않았고 08:10부터 일제히 정상 주기로 돌아왔다). 그러니 이른 새벽에만 열리는 좁은 창은 매일 통째로 비어 있을 수 있다. 앞 단계가 실패하면 뒤 단계가 조기 반환하는 사슬에서는 **가장 좁은 창이 사슬 전체의 단일 실패점**이 되므로, 재시도 창을 뒤 단계와 같은 시간대까지 넓혀 기계가 깨어난 뒤 스스로 복구되게 한다. 작업 키가 하루 한 건이면 성공한 뒤의 시도는 실행 claim이 건너뛴다.
