...
4. 사실이 같으면 새 버전을 만들지 않고 `received_at`, `raw_response_id`와 `updated_at`을 최신 근거로 갱신한다. 이미 완료된 KIS 당일 확인이나 충돌을 후속 KRX `pending` 재수집으로 낮추지 않는다.
...
`20260816_0003` 리비전이 필드, CHECK 제약조건, 버전 유일성과 현재 행 부분 유일 인덱스를 생성한다. 저장소는 원본을 append-only로 남기고 동일 사실 재수신, KRX 정정, 낮은 우선순위 출처 충돌과 동기화 오류 기록을 한 트랜잭션에서 처리한다.

KRX 공식 `[01023] 휴장일` OTP·연간 JSON 응답을 수집해 연도별 원본 1건과 전체 날짜 세션을 한 트랜잭션으로 저장한다. 공식 휴장 목록과 KRX가 공시한 토요일·일요일 휴장 규칙에 해당하면 `closed`, 나머지는 정상 `09:00~15:30`의 `open`으로 적재한다. 응답 계약 불일치, 날짜 중복과 부분 범위는 전체 실패로 `api_sync_status`에 기록한다.

KRX 휴장일 응답에 없는 임시 정규장 시간은 [KRX 보도자료](https://open.krx.co.kr/contents/OPN/05/05000000/OPN05000000.jsp)의 목록과 공식 PDF를 사용한다. 초기 지원 유형은 수능일과 연초 개장일이며 PDF에서 대상 거래일, 유가증권·코스닥시장, 주식·ETF 범위, 변경 전 `09:00~15:30`과 변경 후 시간을 모두 확인한다. 일부 과거 연초 PDF처럼 범위 글자가 벡터로 저장돼 텍스트 추출에서 빠지면 연초 공지 제목, KRX 유가증권·코스닥시장 담당부서, 증시 개장식 문구와 증권상품 표 표식을 모두 요구한다. 공지 행, 선택한 PDF 메타데이터와 PDF 원문 Base64를 `raw_api_response`에 저장한다. 알 수 없는 임시 주식시장 공지, PDF 누락·중복, 본문 추출 실패, 범위·기준시간 불일치는 전체 범위 수집을 실패시킨다.

연간 기본 세션과 임시 공지 보정은 저장 전에 합성한다. 동일 날짜의 연간 세션을 임시 공지가 교체한 완전한 날짜 범위만 한 트랜잭션으로 저장하므로, 후속 연간 재수집도 공지를 다시 적용하며 단축장을 정상장으로 낮추지 않는다.

KIS 공식 저장소의 국내휴장일조회 `CTCA0903R`에서 요청일 행 하나의 `opnd_yn`만 당일 보완 근거로 사용한다. KRX 사실과 같으면 `confirmed`, 다르면 사실 버전을 바꾸지 않고 `conflict`로 만든다. 이 TR은 실전 전용이며 모의환경에서는 호출하지 않는다. 자동 스케줄러는 승인된 [ADR-0006](../decisions/0006-market-calendar-scheduling.md)의 실행 시각과 PostgreSQL 영속 실행 claim을 구현·검증하기 전까지 연결하지 않는다.
